    ...
    def train(self, comment = ''):
        ...
        try:
            for episode in range(self.n_episodes):
                ...
                self.rnd.reset()
                if not self.trial:
                    self.logger.consolidate(self.steps, self.episode, self.actor_critic, self.actor_critic_optimizer, self.rnd)
    ...
    def test(self, going_back_start = False ):
        ...
        while cond:
            if not going_back_start:
                ...
                state = np.concatenate( (robot_pos ,obs)) 
            ...
            if (final_state == 2) and dist > 7 and self.steps > 100:
                cond = False

        return x_lst, y_lst, th_lst, self.steps, final_state, x0, y0, th0, xf, yf, thf, xg, yg
